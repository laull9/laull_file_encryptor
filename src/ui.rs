...
pub struct FileEncryptorApp {
    ...
    ui_scan_include_input_dirs: bool,
    ui_scan_recursive: bool,
...
impl FileEncryptorApp {
    pub fn new(_ctx: &egui::Context) -> Self {
        ...
        let system_dark_mode = dark_light::detect();
        info!("系统深色模式检测结果: {:?}", system_dark_mode);
        if system_dark_mode.unwrap_or(dark_light::Mode::Light) == dark_light::Mode::Dark {
        ...
        Self {
            ...
            ui_scan_include_input_dirs: false,
            ui_scan_recursive: true,
    ...
    fn init_lock_or_unlock(&mut self) -> Result<(), String>{
        if self.selected_files
            ...
            .is_empty() {
            self.result_message = "请先选择文件或文件夹（或拖动到窗口）".to_string();
        ...
        let manager = Arc::new(
            self.encryptor_method.new_encryptor_manager(
                ...
                file_encryptor::base::ScanOptions {
                    include_input_dirs: self.ui_scan_include_input_dirs,
                    recursive: self.ui_scan_recursive,
                }
            )
        );
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui|{
                        ui.checkbox(&mut self.ui_scan_include_input_dirs, "包含输入的文件夹本身");
                        ui.checkbox(&mut self.ui_scan_recursive, "递归扫描子目录");
                    });
